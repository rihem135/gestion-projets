...
// Récupérer tous les messages envoyés ou reçus par l'admin
$messages = $pdo->prepare("
    SELECT m.*, u1.email AS sender_email, u2.email AS receiver_email
    FROM messages m
    JOIN users u1 ON m.sender_id = u1.id
    JOIN users u2 ON m.receiver_id = u2.id
    WHERE m.sender_id = :admin OR m.receiver_id = :admin
    ORDER BY m.sent_at DESC
");
$messages->execute(['admin' => $admin_id]);
...
<div class="container">
    ...
    <div class="formulaire">
        <a href="dashboard_ad.php" class="close-btn" title="Back to dashboard">&times;</a>
        <h2>Send a Message</h2>
        <form action="messaging_admin.php" method="POST" enctype="multipart/form-data">
...
<select name="receiver_id" id="receiver_id" required>
    ...
    <div class="boite">
        ...
        <?php foreach ($allMessages as $msg): ?>
            <div class="message <?= $msg['sender_id'] == $admin_id ? 'envoye' : 'recu' ?>">
